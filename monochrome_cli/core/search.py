...
class SearchEngine:
    ...
    def get_tidal_token(cls) -> Optional[str]:
        ...
        client_id = config.get("tidal_client_id", "txNoH4kkV41MfH25")
        client_secret = config.get("tidal_client_secret", "dQjy0MinCEvxi1O4UmxvxWnDjt4cgHBPw8ll6nYBk98=")
    ...
    def get_album_tracks(cls, album_id: str) -> List[TrackMetadata]:
        ...
        try:
            url = f"https://api.tidal.com/v1/albums/{album_id}/items?countryCode=US&limit=100"
            req = urllib.request.Request(url, headers={"Authorization": f"Bearer {token}"})
